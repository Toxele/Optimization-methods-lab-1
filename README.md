...
Идея: объединение столбцовой нормализации (для стабилизации масштаба) и последнего слоя с моментумом (для снижения дисперсии) даёт простой, но эффективный оптимизатор, не требующий хранения полных моментов для всех параметров.

Реализация:

1. Для каждого слоя $l = 1, \dots, L$:
   - Вычислить $g_t^{(l)} = \nabla_{\theta^{(l)}} \ell(\theta_t; \xi_t)$,
   - Применить столбцовую нормализацию:
     $$
     \tilde{g}_t^{(l)} = \text{col\_norm}(g_t^{(l)})
     $$

2. Если $l = L$ (последний слой):
   $$
   m_t^{(L)} = \beta \cdot m_{t-1}^{(L)} + (1 - \beta) \cdot \tilde{g}_t^{(L)}
   $$

   $$
   \theta_{t+1}^{(L)} = \theta_t^{(L)} - \eta \cdot m_t^{(L)}
   $$

3. Иначе ($l \ne L$):
   $$
   \theta_{t+1}^{(l)} = \theta_t^{(l)} - \eta \cdot \tilde{g}_t^{(l)}
   $$
